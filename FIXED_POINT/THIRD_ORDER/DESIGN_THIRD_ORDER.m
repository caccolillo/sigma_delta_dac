%% 3rd-Order Sigma-Delta Design, Verification and Fixed-Point Word Length Selection
...
%% relaxed Hinf (e.g. 2.0-3.0) is common to gain noise shaping aggressiveness;
%% start at 1.5 then sweep upward only if SNR is insufficient.
H_inf = 1.5;
...
fprintf('G1=%.10f\n', gs(1));   % CIFB 3rd-order has 1 resonator coefficient
...
fprintf('\n--- Step 3: Coefficient range analysis ---\n');
all_coefs  = [bs(1), as(1), as(2), as(3), cs(1), cs(2), cs(3), gs(1)];
coef_names = {'B1','A1','A2','A3','C1','C2','C3','G1'};
...
snr_loss_limit = 0.5;
N_frac_min_ok  = NaN;
snr_results    = zeros(1,24);

%% 3rd-order needs more fractional bits than 2nd-order because g1 is
%% typically very small (~1e-4) and demands high resolution to keep
%% the resonator pole pair inside the unit circle.
for N_frac = 10:24
    scale_n     = 2^N_frac;
    ...
    gs_q = round(gs * scale_n) / scale_n;

    %% Catch the catastrophic case where g1 rounds to zero.
    %% Without the resonator the NTF reduces to a pure 1/(1-z^-1)^3 shape
    %% which is unstable in 1-bit form.
    if any(gs_q == 0) && any(gs ~= 0)
        fprintf('%8d  %8d  %10s  %8s  G1 rounded to zero\n',...
            N_frac, N_total,'---','---');
        continue;
    end

    try
        ABCD_q = stuffABCD(as_q, gs_q, bs_q, cs_q, form);
...
if isnan(N_frac_min_ok)
    error('No sufficient word length found in range 10-24 bits');
end
...
gs_fp = round(gs * scale_fp) / scale_fp;

ABCD_fp  = stuffABCD(as_fp, gs_fp, bs_fp, cs_fp, form);
...
A3_int = int32(round(as(3) * scale_fp));
C1_int = int32(round(cs(1) * scale_fp));
C2_int = int32(round(cs(2) * scale_fp));
C3_int = int32(round(cs(3) * scale_fp));
G1_int = int32(round(gs(1) * scale_fp));
...
for k = 1:N
    ...
    x3_int(k) = double(int3_reg) / scale_fp;
    ...
    C1x1     = int32(int64(C1_int) * int64(int1_reg) / int64(scale_fp));
    int2_reg = int2_reg + C1x1 - fb_A2;
    int2_reg = max(int2_reg, acc_min);
    int2_reg = min(int2_reg, acc_limit);

    %% Integrator 3: int3 += C2*int2 - G1*int3 - A3*v
    %% G1 implements the resonator local feedback that places NTF zeros
    %% at non-DC frequencies inside the signal band.
    C2x2     = int32(int64(C2_int) * int64(int2_reg) / int64(scale_fp));
    G1x3     = int32(int64(G1_int) * int64(int3_reg) / int64(scale_fp));
    int3_reg = int3_reg + C2x2 - G1x3 - fb_A3;
    int3_reg = max(int3_reg, acc_min);
    int3_reg = min(int3_reg, acc_limit);
end
...
fprintf('Max int3 swing         : %.4f  (limit: %.4f)\n',...
    max(abs(x3_int)), double(acc_limit)/scale_fp);
...
fprintf('A3: float=%.10f  fixed=%.10f  int=%8d  error=%.2e\n',...
    as(3), double(A3_int)/scale_fp, double(A3_int), as(3)-double(A3_int)/scale_fp);
...
fprintf('C3: float=%.10f  fixed=%.10f  int=%8d  error=%.2e\n',...
    cs(3), double(C3_int)/scale_fp, double(C3_int), cs(3)-double(C3_int)/scale_fp);
fprintf('G1: float=%.10f  fixed=%.10f  int=%8d  error=%.2e\n',...
    gs(1), double(G1_int)/scale_fp, double(G1_int), gs(1)-double(G1_int)/scale_fp);
...
fprintf('parameter signed [%d:0] A3 = %d''sd%d;\n',...
    N_word_coef-1, N_word_coef, double(A3_int));
...
fprintf('parameter signed [%d:0] C3 = %d''sd%d;\n',...
    N_word_coef-1, N_word_coef, double(C3_int));
fprintf('parameter signed [%d:0] G1 = %d''sd%d;\n',...
    N_word_coef-1, N_word_coef, double(G1_int));
fprintf('\n');
fprintf('reg signed [%d:0] int1_reg;  // accumulator 1\n', N_word_acc-1);
fprintf('reg signed [%d:0] int2_reg;  // accumulator 2\n', N_word_acc-1);
fprintf('reg signed [%d:0] int3_reg;  // accumulator 3\n', N_word_acc-1);

%% ---------------------------------------------------------------
%  PART 5: PLOTS
% ---------------------------------------------------------------
figure('Name','3rd-Order SDM Word Length Selection','NumberTitle','off');
...
xlabel('Frequency (kHz)');
ylabel('|NTF| (dB)');
title('Noise Transfer Function (3rd order)');
grid on;
...
% ---------------------------------------------------------------
fprintf('\n======================================================\n');
fprintf('FIXED-POINT WORD LENGTH SUMMARY (3rd order)\n');
fprintf('======================================================\n');
...
%% Auto-generate SDM_fcn.m as a FLOATING-POINT reference implementation.
%% This function is intended as the input to Fixed-Point Designer
%% (fiaccel / Fixed-Point Tool / "Convert to Fixed Point" workflow).
%% Keep all arithmetic in plain double precision so Fixed-Point Designer
%% can propose word lengths from instrumented simulation runs.
%%
%% Recommended Fixed-Point Designer settings to start from:
%%   Input u                     : fixdt(1, %d, %d)   (matches %d-bit signed input)
%%   Coefficients B1,A1..A3,C1,C2,G1 : fixdt(1, %d, %d)
%%   Integrator states int1,int2,int3 : fixdt(1, %d, %d)
%% Then let "Propose Fraction Lengths" / "Propose Word Lengths" refine.
...
fprintf(fid, '%%\n');
fprintf(fid, '%% This file is the floating-point golden model intended for use with\n');
fprintf(fid, '%% MATLAB Fixed-Point Designer ("Convert to Fixed Point" workflow).\n');
fprintf(fid, '%% All arithmetic uses double precision so the tool can instrument the\n');
fprintf(fid, '%% function, observe min/max ranges, and propose data types.\n');
fprintf(fid, '%%\n');
...
fprintf(fid, '%%\n');
fprintf(fid, '%% Suggested starting types for Fixed-Point Designer:\n');
fprintf(fid, '%%   u           : fixdt(1, %d, %d)\n', input_bits, 0);
fprintf(fid, '%%   coefficients: fixdt(1, %d, %d)\n', N_word_coef, N_frac_coef);
fprintf(fid, '%%   int1..int3  : fixdt(1, %d, %d)\n', N_word_acc,  N_frac_coef);
fprintf(fid, '\n');
...
fprintf(fid, 'end\n');
fclose(fid);
fprintf('\nSDM_fcn.m auto-generated as FLOATING-POINT reference\n');
fprintf('  -> Use MATLAB Fixed-Point Designer to convert.\n');
fprintf('  -> Suggested starting types embedded in the file header.\n');