...
// sdm_cifb_3rd_tb.cpp
//
// Vitis HLS testbench for the 3rd-order CIFB SDM.
//
...
int main() {
    ...
    const double F_SIG      = 1019.3;      // 1019.3 Hz test tone
    const double AMP        = 1024.0;      // 25% full scale (= -12 dBFS)

    const double SIM_TIME_S = 100.0e-3;     // 100 ms simulation
    ...
    //  PRINT CONFIGURATION
    // ----------------------------------------------------------------
    std::cout << "===== 3rd-order SDM HLS testbench =====" << std::endl;
    ...
    for (int n = 0; n < TOTAL_CLOCKS; n++) {
        ...
        // Run the SDM
        pdm_t pdm = sdm_cifb_3rd(u_curr, valid);
